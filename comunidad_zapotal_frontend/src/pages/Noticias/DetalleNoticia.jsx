...
function DetalleNoticia() {
  ...
  useEffect(() => {
    ...
    const singular = `/noticia/detalle/${noticiaId}/relacionadas/${catParam}`;
    const plural   = `/noticias/${noticiaId}/relacionadas/${catParam}`;
    ...
    api.get(singular)
      ...
      .catch(err => {
        if (err?.response?.status === 404) {
          api.get(plural).then(res => apply(res.data)).catch(() => setRelacionadas({ grupos: [] }));
        } else {
          setRelacionadas({ grupos: [] });
        }
      });
          ...
          <div className="noticia-card">
            ...
            <div className={`yt-desc-box ${descExpanded ? "expandida" : ""}`}>
              <div className="yt-desc-meta-row">
                <span className="yt-desc-views"><FaEye /> {(noticia.vistas ?? 0)} visualizaciones</span>
